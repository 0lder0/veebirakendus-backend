...
router.patch("/muuda-aktiivsus-neg", (req: Request, res: Response) => {
for (let index = 0; index < tooted.length; index++) {
        tooted[index].isActive = !tooted[index].isActive;
    }
    res.send(tooted)
});
...
router.get("/suurim-hind", (req: Request, res: Response) => {
    let suurimHind: number = 0;
    for (let index = 0; index < tooted.length; index++) {
        if (tooted[index].price > suurimHind) {
            suurimHind = tooted[index].price;
        }
    }
    res.send("Suurim hind on: " + suurimHind)
});
